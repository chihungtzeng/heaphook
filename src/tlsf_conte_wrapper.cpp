...
namespace heaphook
{
constexpr bool LOG_ALLOCATION = true;
constexpr size_t DEFAULT_MEMORY_POOL_SIZE = 1 << 20;

TLSFConteWrapper::TLSFConteWrapper()
{
}

TLSFConteWrapper::TLSFConteWrapper(size_t init_pool_size) : num_mmap_areas_(0)
{
...
void * TLSFConteWrapper::do_alloc(size_t bytes)
{
  ...
  if (ptr) {
    tlsf_block_set_allocator_index(ptr, allocator_index_);
  }
  return ptr;
}
...
void * TLSFConteWrapper::do_realloc(void * ptr, size_t new_size)
{
  ...
  if (new_ptr) {
    tlsf_block_set_allocator_index(new_ptr, allocator_index_);
  }
  return new_ptr;
}
...
bool TLSFConteWrapper::ptr_is_mine(void * ptr)
{
...
int TLSFConteWrapper::set_allocator_index(uint32_t index)
{
  allocator_index_ = index;
  return 0;
}
  ...
  if (ptr != MAP_FAILED) {
    ...
    if (LOG_ALLOCATION) {
      ...
      fprintf(
        ...
        getpid(), gettid(), length, allocator_index_, total_bytes, total_bytes >> 20);
    }
    return 0;
  } else {
    if (LOG_ALLOCATION) {
